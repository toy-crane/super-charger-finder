export interface SuperCharger {
  id: number
  region?: "서울" | "경기" | "강원" | "충청" | "경상" | "전라" | "제주"
  name: string
  freeParkingChargeTime: number | "free"
  parkingFee: string | null
  parkingFeeDiscount: string | null
  chargingSpeed?: "120W" | "250W"
  DcCount?: number
  locationId: string
  stallCount: number
  dateOpened?: string
  counted?: boolean
  elevationMeters?: number
  powerKilowatt?: number
  solarCanopy?: boolean
  battery?: boolean
  statusDays?: number
  urlDiscuss?: boolean
  status: string
  address: {
    streetName: string
    city: string
    locationDetail: string
    state: string | null
    shortState: string
    zip?: string | null
    countryId?: number
    country?: string
    regionId?: number
    region?: string
  }
  gps: {
    latitude: number
    longitude: number
  }
  KRName: string
  commonName: string
}

const superChargerInfo: SuperCharger[] = [
  {
    id: 1,
    locationId: "seoulgangseosupercharger",
    name: "Seoul - Gangseo, South Korea",
    status: "OPEN",
    address: {
      city: "강서구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 강서구 하늘길 38",
      locationDetail: "지하 1층 B15",
    },
    gps: {
      latitude: 37.563265,
      longitude: 126.803108,
    },
    dateOpened: "2019-12-20",
    stallCount: 4,
    counted: true,
    elevationMeters: 13,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "서울–강서 수퍼차저",
    commonName: "롯데몰 김포공항점",
    parkingFee: "초과 30분당 1,000원",
    freeParkingChargeTime: 30,
    parkingFeeDiscount: "1만원 구매 시, 1시간 무료",
  },
  {
    id: 2,
    locationId: "seoulteheransupercharger",
    name: "Seoul - Tehran, South Korea",
    status: "OPEN",
    address: {
      city: "강남구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 강남구 테헤란로 129",
      locationDetail: "지하 4층",
    },
    gps: {
      latitude: 37.499873,
      longitude: 127.032354,
    },
    dateOpened: "2019-12-31",
    stallCount: 4,
    counted: true,
    elevationMeters: 45,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "서울 – 테헤란 수퍼차저",
    commonName: "강남N타워",
    freeParkingChargeTime: 0,
    parkingFee: "30분 3,000원 추가 10분당 1,000원",
    parkingFeeDiscount: "업장 이용 시 2시간 무료",
    DcCount: undefined,
  },
  {
    id: 3,
    locationId: "seoulgangnamsupercharger",
    name: "Seoul - Gangnam, South Korea",
    status: "OPEN",
    parkingFee: "기본 30분 4,500원 \n 이후 10분당 1,500원",
    parkingFeeDiscount:
      "호텔 1층 그랜드 델리 2만원 구입 시, 2시간 무료 주차 \n 코엑스몰 식당 할인 안됨",
    address: {
      city: "강남구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 강남구 테헤란로 521",
      locationDetail: "지하 4층",
    },
    gps: {
      latitude: 37.509215,
      longitude: 127.06079,
    },
    dateOpened: "2017-05-31",
    stallCount: 5,
    counted: true,
    elevationMeters: 30,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "서울 - 강남 파르나스 수퍼차저",
    commonName: "그랜드 인터컨티넨탈 서울 파르나스",
    freeParkingChargeTime: 20,
    DcCount: undefined,
  },
  {
    id: 4,
    locationId: "seouljongnosupercharger",
    name: "Seoul - Jongno, South Korea",
    status: "OPEN",
    address: {
      city: "종로구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 종로구 종로 33",
      locationDetail: "지하 6층",
    },
    gps: {
      latitude: 37.570693,
      longitude: 126.981511,
    },
    dateOpened: "2017-06-12",
    stallCount: 6,
    counted: true,
    elevationMeters: 45,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "서울 - 종로 수퍼차저",
    commonName: "그랑서울",
    freeParkingChargeTime: 10,
    parkingFee: "평일 10분당 1,000원 \n 주말 1시간 1,000원 12시간 5,000원",
    parkingFeeDiscount:
      "카페, 편의시설 이용 시 1시간 무료 \n 식당 이용 시 2시간 무료",
    DcCount: 3,
  },
  {
    id: 5,
    locationId: "seoulyeouidosupercharger",
    name: "Seoul - Yeoido, South Korea",
    status: "OPEN",
    address: {
      city: "영등포구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 영등포구 국제금융로 10",
      locationDetail: "지하 7층 #64 기둥",
    },
    parkingFee:
      "초과 10분당 1,000원 \n 2만원 이상 1시간 무료 \n 4만원 이상 2시간 무료",
    parkingFeeDiscount: "1만원 구매 시 1시간 무료",
    gps: {
      latitude: 37.52526,
      longitude: 126.925714,
    },
    dateOpened: "2017-06-19",
    stallCount: 5,
    counted: true,
    elevationMeters: 17,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "서울 - 여의도 수퍼차저",
    commonName: "IFC 몰 & 콘래드 서울 호텔",
    freeParkingChargeTime: 30,
    DcCount: undefined,
  },
  {
    id: 6,
    locationId: "seouljamsilsupercharger",
    name: "Seoul - Jamsil, South Korea",
    status: "OPEN",
    address: {
      city: "송파구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 송파구 올림픽로 300",
      locationDetail: "지하 3층",
    },
    gps: {
      latitude: 37.513395,
      longitude: 127.102238,
    },
    dateOpened: "2017-10-09",
    stallCount: 5,
    counted: true,
    elevationMeters: 23,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "서울 - 잠실 수퍼차저",
    commonName: "롯데월드타워",
    freeParkingChargeTime: 0,
    parkingFee: "10시 ~ 20시: 10분당 500원 \n 그 외 시간: 10분당 200원",
    parkingFeeDiscount: "문화 시설 이외에는 주차권 미지급",
    DcCount: undefined,
  },
  {
    id: 7,
    locationId: "seoulyongsansupercharger",
    name: "Seoul - Yongsan, South Korea",
    status: "OPEN",
    address: {
      city: "용산구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 용산구 소월로 322",
      locationDetail: "지하 2층",
    },
    gps: {
      latitude: 37.539341,
      longitude: 126.997137,
    },
    dateOpened: "2017-12-22",
    stallCount: 5,
    counted: true,
    elevationMeters: 104,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "서울 - 용산 수퍼차저",
    commonName: "그랜드 하얏트",
    freeParkingChargeTime: 15,
    parkingFee: "30분 당 4,000원 이후 10분당 2,5000원",
    parkingFeeDiscount: "투숙객 할인 안됨",
    DcCount: undefined,
  },
  {
    id: 8,
    freeParkingChargeTime: 15,
    parkingFee: "기본 30분 4,000원 \n 이후 10분당 2,000원",
    locationId: "gangnnamsupercharger",
    name: "Seoul - Apgujeong, South Korea",
    status: "OPEN",
    address: {
      city: "강남구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 강남구 논현로 854",
      locationDetail: "지하 4층",
    },
    gps: {
      latitude: 37.52541809,
      longitude: 127.02887026,
    },
    dateOpened: "2019-09-11",
    stallCount: 4,
    counted: true,
    elevationMeters: 32,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "서울 - 압구정 수퍼차저",
    commonName: "안다즈 서울 강남",
    parkingFeeDiscount: null,
  },
  {
    id: 9,
    locationId: "gurosupercharger",
    name: "Seoul - Guro, South Korea",
    status: "OPEN",
    address: {
      city: "구로구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 구로구 디지털로26길 38",
      locationDetail: "지하 4층",
    },
    gps: {
      latitude: 37.47984,
      longitude: 126.8953,
    },
    dateOpened: "2021-06-17",
    stallCount: 9,
    counted: true,
    elevationMeters: 18,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "서울 - 구로 수퍼차저",
    commonName: "지타워",
    freeParkingChargeTime: 15,
    parkingFee: "10분당 700원",
    parkingFeeDiscount: "업장 이용 시 1시간 무료",
    DcCount: 3,
  },
  {
    id: 10,
    locationId: "seolleungsupercharger",
    name: "Seoul - Centerfield, South Korea",
    status: "OPEN",
    address: {
      city: "강남구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 강남구 테헤란로 231",
      locationDetail: "지하 5층",
    },
    gps: {
      latitude: 37.503216,
      longitude: 127.04162,
    },
    dateOpened: "2021-06-24",
    stallCount: 6,
    counted: true,
    elevationMeters: 37,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "서울 - 센터필드 수퍼차저",
    commonName: "센터필드",
    freeParkingChargeTime: 30,
    parkingFee: "10분당 1,500원",
    parkingFeeDiscount: "업장 이용 시 2시간 무료 \n 스타벅스 1시간 무료",
    DcCount: 12,
  },
  {
    id: 11,
    locationId: "seongdongsupercharger",
    name: "Seoul - Wangsimni, South Korea",
    status: "OPEN",
    address: {
      city: "성동구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 성동구 왕십리광장로 17",
      locationDetail: "지하 5층",
    },
    gps: {
      latitude: 37.5604,
      longitude: 127.0386,
    },
    dateOpened: "2021-06-25",
    stallCount: 6,
    counted: true,
    elevationMeters: 18,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "서울 - 왕십리 수퍼차저",
    commonName: "비트플렉스",
    freeParkingChargeTime: 30,
    parkingFee: "10분당 1,000원",
    parkingFeeDiscount:
      "3만원 미만 구매 시 1시간 무료 \n 3만원 이상 구매 시 2시간 무료",
    DcCount: 3,
  },
  {
    id: 12,
    locationId: "seoulseochosupercharger",
    name: "Seoul - Seocho, South Korea",
    status: "OPEN",
    address: {
      city: "서초구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 서초구 서초중앙로 18",
      locationDetail: "지하 2층",
    },
    gps: {
      latitude: 37.48406,
      longitude: 127.01732,
    },
    dateOpened: "2021-07-01",
    stallCount: 6,
    counted: true,
    elevationMeters: 40,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "서울 - 서초 수퍼차저",
    commonName: "서초쌍용플래티넘",
    freeParkingChargeTime: 5,
    parkingFee: "기본 5,000원 \n 50분 초과 시, 10분당 1,000원",
    parkingFeeDiscount: null,
    DcCount: 8,
  },
  {
    id: 13,
    locationId: "gangnamsinsasupercharger",
    name: "Seoul - Sinsa, South Korea",
    status: "OPEN",
    address: {
      city: "강남구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울 강남구 강남대로 652",
      locationDetail: "지하 4층",
    },
    gps: {
      latitude: 37.519982,
      longitude: 127.018977,
    },
    dateOpened: "2022-05-16",
    stallCount: 3,
    counted: true,
    elevationMeters: 20,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "서울-신사 수퍼차저",
    commonName: "신사스퀘어",
    freeParkingChargeTime: 30,
    parkingFee: "30분당 3,000원 추가 10분당 1,000원",
    parkingFeeDiscount: null,
    DcCount: undefined,
  },
  {
    id: 14,
    locationId: "gangbuksupercharger",
    name: "Gangbuk, South Korea",
    status: "OPEN",
    address: {
      city: "강북구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 강북구 삼양로 689",
      locationDetail: "지상 1,2층",
    },
    gps: {
      latitude: 37.66375,
      longitude: 127.00871,
    },
    dateOpened: "2021-09-11",
    stallCount: 6,
    counted: true,
    elevationMeters: 69,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "강북 수퍼차저",
    commonName: "파라스파라 서울",
    freeParkingChargeTime: 30,
    parkingFee: "10분당 1,000원",
    parkingFeeDiscount: "업장 이용 시 3시간 무료",
    DcCount: undefined,
  },
  {
    id: 15,
    locationId: "seoulccmmsupercharger",
    name: "Seoul - CCMM Building, South Korea",
    status: "OPEN",
    address: {
      city: "영등포구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 영등포구 여의공원로 101",
      locationDetail: "지하 6층",
    },
    gps: {
      latitude: 37.529396,
      longitude: 126.924755,
    },
    dateOpened: "2022-06-14",
    stallCount: 6,
    counted: true,
    elevationMeters: 11,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "CCMM 수퍼차저",
    commonName: "CCMM빌딩",
    freeParkingChargeTime: 10,
    parkingFee: "10분당 1,000원",
    parkingFeeDiscount: null,
    DcCount: undefined,
  },
  {
    id: 16,
    locationId: "songpasupercharger",
    name: "Seoul - Songpa, South Korea",
    status: "OPEN",
    address: {
      city: "송파구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 송파구 잠실로 209",
      locationDetail: "지하 4층",
    },
    gps: {
      latitude: 37.513534,
      longitude: 127.106174,
    },
    dateOpened: "2021-09-25",
    stallCount: 6,
    counted: true,
    elevationMeters: 15,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "송파 수퍼차저",
    commonName: "소피텔 앰배서더 서울",
    freeParkingChargeTime: 25,
    parkingFee: "10분당 1,000원",
    parkingFeeDiscount: "업장 이용 시 1-2시간 무료 (스타벅스 제외)",
    DcCount: undefined,
  },
  {
    id: 17,
    locationId: "seoulsongpaolympicparksupercharger",
    name: "Seoul - Jamsil B, South Korea",
    status: "OPEN",
    address: {
      city: "송파구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 송파구 올림픽로 240",
      locationDetail: "지하 3층 D3999",
    },
    gps: {
      latitude: 37.511499,
      longitude: 127.098247,
    },
    dateOpened: "2022-07-02",
    stallCount: 6,
    counted: true,
    elevationMeters: 14,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "서울 – 잠실B 수퍼차저",
    commonName: "롯데백화점 잠실점",
    freeParkingChargeTime: 0,
    parkingFee: "기본 30분 1,000원 \n 초과 10분당 1,000원",
    parkingFeeDiscount: "1/3/5만원 마트 구매 시 1/2/3시간 무료",
    DcCount: undefined,
  },
  {
    id: 18,
    locationId: "suyusupercharger",
    name: "Suyu, South Korea",
    status: "OPEN",
    address: {
      city: "강북구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 강북구 도봉로 315",
      locationDetail: "지하 4층",
    },
    gps: {
      latitude: 37.637192,
      longitude: 127.023378,
    },
    dateOpened: "2022-06-14",
    stallCount: 6,
    counted: true,
    elevationMeters: 32,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "수유 수퍼차저",
    commonName: "에피소드 수유 838",
    freeParkingChargeTime: 10,
    parkingFee: "15분당 1,000원",
    parkingFeeDiscount: null,
    DcCount: 2,
  },
  {
    id: 19,
    locationId: "seoulcheongnyangrisupercharger",
    name: "Seoul - Cheongnyangni, South Korea",
    status: "OPEN",
    address: {
      city: "동대문구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 동대문구 왕산로 214",
      locationDetail: "M5F",
    },
    gps: {
      latitude: 37.579635,
      longitude: 127.046048,
    },
    dateOpened: "2021-11-07",
    stallCount: 3,
    counted: true,
    elevationMeters: 16,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "청량리 수퍼차저",
    commonName: "롯데백화점 청량리점",
    freeParkingChargeTime: 30,
    parkingFee: "30분당 1,000원, 10분당 1,000원",
    parkingFeeDiscount:
      "1/5/10만원 구매 시, 1/2/3시간 무료 \n 영화 3시간 \n 식당 2시간",
    DcCount: 6,
  },
  {
    id: 20,
    locationId: "seongsudtowerq322",
    name: "Seoul - Seongsu D Tower, South Korea",
    status: "OPEN",
    address: {
      streetName: "B5, 83-21 Wangsimni-ro, Seongdong-gu. SEOUL, 04769",
      city: "성동구",
      state: "서울특별시",
      shortState: "서울",
      zip: "04769",
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      locationDetail: "지하 5층",
    },
    gps: {
      latitude: 37.544332,
      longitude: 127.04416,
    },
    dateOpened: "2022-11-10",
    stallCount: 9,
    counted: true,
    elevationMeters: 11,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    commonName: "성수 D타워",
    KRName: "성수 슈퍼차저",
    freeParkingChargeTime: 20,
    parkingFee: "30분 3,000원 추가 10분당 1,000원",
    parkingFeeDiscount: "편의점만 사용해도 2시간 무료 주차",
    DcCount: 3,
  },
  {
    id: 21,
    locationId: "seoultimesstreamsupercharger",
    name: "Seoul - Times Stream, South Korea",
    status: "OPEN",
    address: {
      city: "관악구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 관악구 신림로 330",
      locationDetail: "지하 6층",
    },
    gps: {
      latitude: 37.4838519,
      longitude: 126.930193,
    },
    dateOpened: "2022-06-13",
    stallCount: 6,
    counted: true,
    elevationMeters: 22,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "서울 – 타임스트림 수퍼차저",
    commonName: "타임스트림",
    freeParkingChargeTime: 20,
    parkingFee: "최초 30분 1,000원, 10분당 1,000원",
    parkingFeeDiscount: "1/3/5만원 구매 시, 1/2/3시간 무료",
    DcCount: 2,
  },
  {
    id: 22,
    locationId: "geumcheonsupercharger",
    name: "Geumchon, South Korea",
    status: "OPEN",
    address: {
      city: "금천구",
      state: "서울특별시",
      shortState: "서울",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "서울특별시 금천구 디지털로 185",
      locationDetail: "1관 지하 3층",
    },
    gps: {
      latitude: 37.478,
      longitude: 126.8872,
    },
    dateOpened: "2022-09-30",
    stallCount: 9,
    counted: true,
    elevationMeters: 8,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "금천 마리오 수퍼차저",
    commonName: "마리오아울렛 1관",
    freeParkingChargeTime: 30,
    parkingFee: "30분 이후 10분당 500원",
    parkingFeeDiscount: "5/10/20/30/40/50만원 구매 시, 1/2/3/4/5/6시간 무료",
    DcCount: undefined,
  },
  {
    id: 23,
    locationId: "hanamsupercharger",
    name: "Hanam, South Korea",
    status: "OPEN",
    address: {
      city: "하남시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 하남시 미사대로 750",
      locationDetail: "지상 4층 12C",
    },
    gps: {
      latitude: 37.545551,
      longitude: 127.223682,
    },
    dateOpened: "2019-12-24",
    stallCount: 6,
    counted: true,
    elevationMeters: 25,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "하남 수퍼차저",
    commonName: "스타필드 하남",
    freeParkingChargeTime: "free",
    parkingFeeDiscount: null,
    parkingFee: null,
  },
  {
    id: 24,
    locationId: "gapyeongsupercharger",
    name: "Gapyeong, South Korea",
    status: "OPEN",
    address: {
      city: "가평군",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 가평군 설악면 미사리로 3",
      locationDetail: "지상",
    },
    gps: {
      latitude: 37.686451,
      longitude: 127.49363,
    },
    dateOpened: "2019-12-24",
    stallCount: 6,
    counted: true,
    elevationMeters: 63,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "가평 수퍼차저",
    commonName: "설악 IC 가평",
    freeParkingChargeTime: 0,
    parkingFee: "30분 2,000원 추가 10분당 1,000원",
    parkingFeeDiscount: null,
  },
  {
    id: 25,
    locationId: "bundangsupercharger",
    name: "Seongnam - Bundang, South Korea",
    status: "OPEN",
    address: {
      city: "분당구",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 성남시 분당구 탄천상로151번길 20",
      locationDetail: "지하 4층 E6",
    },
    gps: {
      latitude: 37.340546,
      longitude: 127.106848,
    },
    dateOpened: "2019-10-02",
    stallCount: 6,
    counted: true,
    elevationMeters: 71,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "성남–분당 수퍼차저",
    commonName: "CGV스퀘어 오리점",
    freeParkingChargeTime: 60,
    parkingFee: null,
    parkingFeeDiscount: "홈플러스 이용 시 2시간 무료 \n CGV 이용 시 3시간 무료",
  },
  {
    id: 26,
    locationId: "suwonsupercharger",
    name: "Suwon, South Korea",
    status: "OPEN",
    address: {
      city: "수원시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 수원시 권선구 세화로 134",
      locationDetail: "지하 2층 L1",
    },
    gps: {
      latitude: 37.264011,
      longitude: 126.996636,
    },
    dateOpened: "2018-03-27",
    stallCount: 6,
    counted: true,
    elevationMeters: 35,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "수원 수퍼차저",
    commonName: "롯데몰 수원점",
    freeParkingChargeTime: 30,
    parkingFee: "10분당 500원",
    parkingFeeDiscount: "1만원 구매 시, 1시간 무료",
  },
  {
    id: 27,
    locationId: "seongnampangyosupercharger",
    name: "Seognam - Pangyo, South Korea",
    status: "OPEN",
    address: {
      city: "성남시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 성남시 분당구 판교로319번길 13",
      locationDetail: "지상 4층",
    },
    gps: {
      latitude: 37.403642,
      longitude: 127.109051,
    },
    dateOpened: "2017-12-22",
    stallCount: 4,
    counted: true,
    elevationMeters: 73,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "성남 판교 수퍼차저",
    commonName: "판교 디테라스",
    parkingFee: "20분당 1,000원",
    parkingFeeDiscount:
      "평일 업장 이용 시, 1~2시간 무료 \n 휴일 업장 이용 시 무료 ",
    DcCount: 1,
    freeParkingChargeTime: 0,
  },
  {
    id: 28,
    locationId: "pyeongtaeksupercharger",
    name: "Pyeongtaek, South Korea",
    status: "OPEN",
    address: {
      city: "평택시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 평택시 청원로 1409",
      locationDetail: "지상",
    },
    gps: {
      latitude: 37.053481,
      longitude: 127.08031,
    },
    dateOpened: "2021-12-15",
    stallCount: 8,
    counted: true,
    elevationMeters: 29,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "평택 수퍼차저",
    commonName: "송탄 블루에너지 충전소",
    freeParkingChargeTime: "free",
    parkingFee: null,
    parkingFeeDiscount: null,
  },
  {
    id: 29,
    locationId: "goyangsupercharger",
    name: "Goyang, South Korea",
    status: "OPEN",
    address: {
      city: "고양시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 고양시 일산동구 태극로 20",
      locationDetail: "지하 3층",
    },
    gps: {
      latitude: 37.661945,
      longitude: 126.750771,
    },
    dateOpened: "2018-03-03",
    stallCount: 4,
    counted: true,
    elevationMeters: 7,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "고양 수퍼차저",
    commonName: "소노캄 고양",
    freeParkingChargeTime: 10,
    DcCount: 2,
    parkingFee: "30분 1,000원이후 15분마다 1,000원",
    parkingFeeDiscount: "1층 스타벅스 이용 시 1시간 무료",
  },
  {
    id: 30,
    locationId: "gimposupercharger",
    name: "Gimpo, South Korea",
    status: "OPEN",
    address: {
      city: "김포시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 김포시 김포한강4로 487",
      locationDetail: "지하 1층",
    },
    gps: {
      latitude: 37.645455,
      longitude: 126.63037,
    },
    dateOpened: "2021-12-24",
    stallCount: 6,
    counted: true,
    elevationMeters: 15,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "김포 수퍼차저",
    commonName: "카림애비뉴 김포",
    DcCount: 3,
    parkingFee: "최초 30분 1,000원 추가 30분당 500원",
    parkingFeeDiscount: "상가 이용 시 1시간 무료",
    freeParkingChargeTime: 0,
  },
  {
    id: 31,
    locationId: "ilsansupercharger",
    name: "Ilsan, South Korea",
    status: "OPEN",
    address: {
      city: "고양시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 고양시 일산동구 장항로 31",
      locationDetail: "지상",
    },
    gps: {
      latitude: 37.629688,
      longitude: 126.791093,
    },
    dateOpened: "2021-03-30",
    stallCount: 8,
    counted: true,
    elevationMeters: 8,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "일산 수퍼차저",
    commonName: "N2WASH 일산전기차충전소",
    freeParkingChargeTime: 5,
    parkingFeeDiscount: "커피/세차 이용 시 무료",
    parkingFee: "10분 600원",
  },
  {
    id: 32,
    locationId: "yonginsupercharger",
    name: "Yongin-Giheung, South Korea",
    status: "OPEN",
    address: {
      city: "용인시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 용인시 기흥구 신고매로 124",
      locationDetail: "지하 3층 A5",
    },
    gps: {
      latitude: 37.225032,
      longitude: 127.123109,
    },
    dateOpened: "2021-03-30",
    stallCount: 6,
    counted: true,
    elevationMeters: 116,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "용인-기흥 수퍼차저",
    commonName: "롯데프리미엄아울렛 기흥점",
    freeParkingChargeTime: "free",
    DcCount: 6,
    parkingFee: null,
    parkingFeeDiscount: null,
  },
  {
    id: 33,
    locationId: "yeojusupercharger",
    name: "Yeoju, South Korea",
    status: "OPEN",
    address: {
      city: "여주시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 여주시 명품로 360",
      locationDetail: "지상 A",
    },
    gps: {
      latitude: 37.240451,
      longitude: 127.613829,
    },
    dateOpened: "2021-03-30",
    stallCount: 8,
    counted: true,
    elevationMeters: 113,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "여주 수퍼차저",
    commonName: "여주 신세계 프리미엄 아울렛",
    DcCount: 12,
    freeParkingChargeTime: "free",
    parkingFee: null,
    parkingFeeDiscount: null,
  },
  {
    id: 34,
    locationId: "hwasungsupercharger",
    name: "Hwaseong, South Korea",
    status: "OPEN",
    address: {
      city: "화성시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 화성시 향남읍 서해로 806",
      locationDetail: "지상",
    },
    gps: {
      latitude: 37.13485,
      longitude: 126.895718,
    },
    dateOpened: "2021-04-30",
    stallCount: 6,
    counted: true,
    elevationMeters: 40,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "화성 수퍼차저",
    commonName: "화성 GS 주유소",
    freeParkingChargeTime: "free",
    parkingFee: null,
    parkingFeeDiscount: null,
  },
  {
    id: 35,
    locationId: "anseongsupercharger",
    name: "Anseong, South Korea",
    status: "OPEN",
    address: {
      city: "안성시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 안성시 안성맞춤대로 833",
      locationDetail: "지상 1층",
    },
    gps: {
      latitude: 36.988973,
      longitude: 127.265732,
    },
    dateOpened: "2022-07-02",
    stallCount: 6,
    counted: true,
    elevationMeters: 41,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "안성 수퍼차저",
    commonName: "안성 축협",
    freeParkingChargeTime: "free",
    parkingFee: null,
    parkingFeeDiscount: null,
  },
  {
    id: 36,
    locationId: "dongtansupercharger",
    name: "Dongtan, South Korea",
    status: "OPEN",
    address: {
      city: "화성시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 화성시 동탄역로 160",
      locationDetail: "지하 4층 E6",
    },
    gps: {
      latitude: 37.191169,
      longitude: 127.096183,
    },
    dateOpened: "2021-08-19",
    stallCount: 3,
    counted: true,
    elevationMeters: 36,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "동탄 수퍼차저",
    commonName: "롯데백화점 동탄점",
    freeParkingChargeTime: 30,
    parkingFee: "10분당 1,000원",
    parkingFeeDiscount: "1/3/5만원 1/2/3시간 무료",
    DcCount: 3,
  },
  {
    id: 37,
    locationId: "euiwangsupercharger",
    name: "Uiwang, South Korea",
    status: "OPEN",
    address: {
      city: "의왕시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 의왕시 학의동 1039",
      locationDetail: "지하 1층",
    },
    gps: {
      latitude: 37.375635,
      longitude: 127.00947,
    },
    dateOpened: "2021-09-10",
    stallCount: 3,
    counted: true,
    elevationMeters: 86,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "의왕 수퍼차저",
    commonName: "롯데프리미엄아울렛 타임빌라스",
    DcCount: 3,
    freeParkingChargeTime: 30,
    parkingFee: "10분당 1,000원",
    parkingFeeDiscount: "1/3/5만원 1/2/3시간 무료",
  },
  {
    id: 38,
    locationId: "gyeonggigwangjusupercharger",
    name: "Gyeonggi - Gwangju, South Korea",
    status: "OPEN",
    address: {
      city: "광주시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 광주시 경충대로 1967",
      locationDetail: "지상",
    },
    gps: {
      latitude: 37.403372,
      longitude: 127.22335,
    },
    dateOpened: "2022-06-14",
    stallCount: 6,
    counted: true,
    elevationMeters: 87,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "경기 - 광주 수퍼차저",
    commonName: "나인블럭 중대동점",
    freeParkingChargeTime: "free",
    parkingFee: null,
    parkingFeeDiscount: null,
  },
  {
    id: 39,
    locationId: "gwanggyolakesupercharger",
    name: "Gwanggyo - Central, South Korea",
    status: "OPEN",
    address: {
      city: "수원시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 수원시 영통구 도청로 10",
      locationDetail: "지하 4층",
    },
    gps: {
      latitude: 37.28668,
      longitude: 127.055814,
    },
    dateOpened: "2022-03-20",
    stallCount: 6,
    counted: true,
    elevationMeters: 60,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "광교 중앙 수퍼차저",
    commonName: "롯데아울렛 광교점",
    freeParkingChargeTime: 39,
    DcCount: 3,
    parkingFee: null,
    parkingFeeDiscount: null,
  },
  {
    id: 40,
    locationId: "seongnamalphadome2supercharger",
    name: "Pangyo - Baekhyun A, South Korea",
    status: "OPEN",
    address: {
      city: "성남시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 성남시 분당구 백현동 534",
      locationDetail: "지하 2층",
    },
    gps: {
      latitude: 37.395597,
      longitude: 127.112063,
    },
    dateOpened: "2022-03-20",
    stallCount: 3,
    counted: true,
    elevationMeters: 41,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "판교-백현A 수퍼차저",
    commonName: "판교 테크원타워",
    freeParkingChargeTime: 20,
    parkingFee: "1일 13,000원",
    parkingFeeDiscount: null,
    DcCount: 21,
  },
  {
    id: 41,
    locationId: "gwangmyeongsupercharger",
    name: "Gwangmyeong, South Korea",
    status: "OPEN",
    address: {
      city: "광명시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 광명시 양지로 7",
      locationDetail: "B5F",
    },
    gps: {
      latitude: 37.41917204,
      longitude: 126.88211751,
    },
    dateOpened: "2021-10-01",
    stallCount: 3,
    counted: true,
    elevationMeters: 23,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "광명 수퍼차저",
    commonName: "유플래닛",
    freeParkingChargeTime: 30,
    DcCount: 7,
    parkingFee: "20분당 2,000원",
    parkingFeeDiscount: "2/3/5/10만원 2/3/4/5 무료 주차",
  },
  {
    id: 42,
    locationId: "gwanggyosupercharger",
    name: "Gwanggyo, South Korea",
    status: "OPEN",
    address: {
      city: "수원시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 수원시 영통구 하동 965-7",
      locationDetail: "2층",
    },
    gps: {
      latitude: 37.293423,
      longitude: 127.070231,
    },
    dateOpened: "2021-10-21",
    stallCount: 3,
    counted: true,
    elevationMeters: 65,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "광교 수퍼차저",
    commonName: "광교 법원 주차장",
    freeParkingChargeTime: 0,
    parkingFee: "30분 2,000원 이후 10분당 1,000원",
    parkingFeeDiscount: null,
    DcCount: 1,
  },
  {
    id: 43,
    locationId: "gyeonggianyangsupercharger",
    name: "Anyang, South Korea",
    status: "OPEN",
    address: {
      city: "안양시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 안양시 동안구 부림동 관평로170번길 33",
      locationDetail: "7층",
    },
    gps: {
      latitude: 37.39301,
      longitude: 126.96228,
    },
    dateOpened: "2021-10-31",
    stallCount: 6,
    counted: true,
    elevationMeters: 34,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "안양 수퍼차저",
    commonName: "CGV 평촌",
    freeParkingChargeTime: 10,
    DcCount: 3,
    parkingFee: "20분당 1,000원",
    parkingFeeDiscount: null,
  },
  {
    id: 44,
    locationId: "gurisupercharger",
    name: "Guri, South Korea",
    status: "OPEN",
    address: {
      city: "구리시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 구리시 경춘북로 230",
      locationDetail: "6층 옥상",
    },
    gps: {
      latitude: 37.6333,
      longitude: 127.1159,
    },
    dateOpened: "2022-09-30",
    stallCount: 8,
    counted: true,
    elevationMeters: 39,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "모다아울렛-구리 수퍼차저",
    commonName: "모다아울렛 구리남양주점",
    freeParkingChargeTime: 0,
    parkingFee: null,
    parkingFeeDiscount: null,
  },
  {
    id: 45,
    locationId: "teslagoyangdonaedongsupercharger",
    name: "Goyang - Donae-dong, South Korea",
    status: "OPEN",
    address: {
      city: "고양시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기 고양시 덕양구 권율대로 449 ",
      locationDetail: "지상 1층",
    },
    gps: {
      latitude: 37.6327,
      longitude: 126.8647,
    },
    dateOpened: "2022-09-30",
    stallCount: 8,
    counted: true,
    elevationMeters: 32,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "고양 도내동 수퍼차저",
    commonName: "북고양 커피명당",
    freeParkingChargeTime: "free",
    parkingFee: "없음",
    parkingFeeDiscount: null,
  },
  {
    id: 46,
    locationId: "teslabucheonlottedepartmentsupercharger",
    name: "Bucheon, South Korea",
    status: "OPEN",
    address: {
      city: "부천시",
      state: "경기도",
      shortState: "경기",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경기도 부천시 길주로 300",
      locationDetail: "지하 4층",
    },
    gps: {
      latitude: 37.5026,
      longitude: 126.775,
    },
    dateOpened: "2022-09-30",
    stallCount: 9,
    counted: true,
    elevationMeters: 12,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "부천 수퍼차저",
    commonName: "롯데백화점 중동점",
    freeParkingChargeTime: 30,
    parkingFee: null,
    parkingFeeDiscount: null,
  },
  {
    id: 47,
    locationId: "songdoincheonsupercharger",
    name: "Songdo, South Korea",
    status: "OPEN",
    address: {
      city: "연수구",
      state: "인천광역시",
      shortState: "인천",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "인천광역시 연수구 송도과학로16번길 33-3",
      locationDetail: "D동 지하 2층",
    },
    gps: {
      latitude: 37.378678,
      longitude: 126.662796,
    },
    dateOpened: "2019-12-24",
    stallCount: 4,
    counted: true,
    elevationMeters: 0,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "연수–송도 수퍼차저",
    commonName: "송도 트리플스트리트",
    freeParkingChargeTime: "free",
    parkingFee: null,
    parkingFeeDiscount: null,
    DcCount: 5,
  },
  {
    id: 48,
    locationId: "cheongnasupercharger",
    name: "Cheongna, South Korea",
    status: "OPEN",
    address: {
      city: "서구",
      state: "인천광역시",
      shortState: "인천",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "인천광역시 서구 청라루비로 76",
      locationDetail: "지하 2층 하",
    },
    gps: {
      latitude: 37.532636,
      longitude: 126.641587,
    },
    dateOpened: "2021-12-31",
    stallCount: 8,
    counted: true,
    elevationMeters: 1,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "청라 수퍼차저",
    commonName: "청라 스퀘어세븐",
    freeParkingChargeTime: "free",
    DcCount: 4,
    parkingFee: null,
    parkingFeeDiscount: null,
  },
  {
    id: 49,
    locationId: "incheonmichuholsupercharger",
    name: "Michuhol, South Korea",
    status: "OPEN",
    address: {
      city: "미추홀구",
      state: "인천광역시",
      shortState: "인천",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "인천광역시 미추홀구 숙골로 88번길 12",
      locationDetail: "상가 B4",
    },
    gps: {
      latitude: 37.470798,
      longitude: 126.665855,
    },
    dateOpened: "2021-09-27",
    stallCount: 6,
    counted: true,
    elevationMeters: 13,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "미추홀 수퍼차저",
    commonName: "미추홀 엘리웨이",
    freeParkingChargeTime: 30,
    parkingFee: "추가 10분당 1,000원",
    parkingFeeDiscount: "업장 이용 시, 1/2/3만원 -> 1/2/3시간 무료",
  },
  {
    id: 50,
    locationId: "dongchunsupercharger",
    name: "Yeonsu - Dongchun, South Korea",
    status: "OPEN",
    address: {
      city: "연수구",
      state: "인천광역시",
      shortState: "인천",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "인천광역시 연수구 아암대로 825-25",
      locationDetail: "지상",
    },
    gps: {
      latitude: 37.3959939,
      longitude: 126.663656,
    },
    dateOpened: "2021-10-01",
    stallCount: 8,
    counted: true,
    elevationMeters: 17,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "연수-동춘 수퍼차저",
    commonName: "연수 더 스탈릿",
    freeParkingChargeTime: 5,
    parkingFee: "1시간 1,000원",
    parkingFeeDiscount: null,
  },
  {
    id: 51,
    locationId: "pyeongchangsupercharger",
    name: "Pyeongchang, South Korea",
    status: "OPEN",
    address: {
      city: "평창군",
      state: "강원도",
      shortState: "강원",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "강원도 평창군 봉평면 태기로 174",
      locationDetail: "지상",
    },
    gps: {
      latitude: 37.586685,
      longitude: 128.322039,
    },
    dateOpened: "2021-11-27",
    stallCount: 9,
    counted: true,
    elevationMeters: 663,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "평창 수퍼차저",
    commonName: "평창 휘닉스파크",
    freeParkingChargeTime: "free",
    parkingFee: null,
    parkingFeeDiscount: null,
    DcCount: 10,
  },
  {
    id: 52,
    locationId: "wonjusupercharger",
    name: "Wonju, South Korea",
    status: "OPEN",
    address: {
      city: "원주시",
      state: "강원도",
      shortState: "강원",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "강원도 원주시 지정면 오크밸리2길 58",
      locationDetail: "지상",
    },
    gps: {
      latitude: 37.435072,
      longitude: 127.818718,
    },
    dateOpened: "2017-06-18",
    stallCount: 6,
    counted: true,
    elevationMeters: 296,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "원주 수퍼차저",
    commonName: "오크밸리 리조트",
    parkingFee: null,
    parkingFeeDiscount: null,
    freeParkingChargeTime: "free",
  },
  {
    id: 53,
    locationId: "sokchogangwondosupercharger",
    name: "Sokcho, South Korea",
    status: "OPEN",
    address: {
      city: "속초시",
      state: "강원도",
      shortState: "강원",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "강원도 속초시 대포항길 186",
      locationDetail: "지하 3층",
    },
    gps: {
      latitude: 38.180856,
      longitude: 128.611593,
    },
    dateOpened: "2017-12-01",
    stallCount: 7,
    counted: true,
    elevationMeters: 38,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "속초 수퍼차저",
    commonName: "롯데 리조트 속초",
    freeParkingChargeTime: "free",
    parkingFee: null,
    parkingFeeDiscount: null,
  },
  {
    id: 54,
    locationId: "gangneungsupercharger",
    name: "Gangneung, South Korea",
    status: "OPEN",
    address: {
      city: "강릉시",
      state: "강원도",
      shortState: "강원",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "강원도 강릉시 해안로 536",
      locationDetail: "지하 1층",
    },
    gps: {
      latitude: 37.80534,
      longitude: 128.904599,
    },
    dateOpened: "2017-11-16",
    stallCount: 6,
    counted: true,
    elevationMeters: 100,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "강릉 수퍼차저",
    commonName: "라카이샌드 파인 리조트",
    freeParkingChargeTime: "free",
    parkingFee: null,
    parkingFeeDiscount: null,
  },
  {
    id: 55,
    locationId: "yangyangsupercharger",
    name: "Yangyang, South Korea",
    status: "OPEN",
    address: {
      city: "양양군",
      state: "강원도",
      shortState: "강원",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "강원도 양양군 손양면 공항로 230",
      locationDetail: "지상",
    },
    gps: {
      latitude: 38.054626,
      longitude: 128.660877,
    },
    dateOpened: "2021-12-31",
    stallCount: 8,
    counted: true,
    elevationMeters: 89,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "양양 수퍼차저",
    commonName: "설해원 리조트",
    freeParkingChargeTime: "free",
    parkingFee: null,
    parkingFeeDiscount: null,
    DcCount: 5,
  },
  {
    id: 56,
    locationId: "chuncheonsupercharger",
    name: "Chuncheon, South Korea",
    status: "OPEN",
    address: {
      city: "춘천시",
      state: "강원도",
      shortState: "강원",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "강원도 춘천시 후석로 120",
      locationDetail: "2층",
    },
    gps: {
      latitude: 37.863274,
      longitude: 127.748106,
    },
    dateOpened: "2022-07-02",
    stallCount: 6,
    counted: true,
    elevationMeters: 98,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "춘천 수퍼차저",
    commonName: "춘천 NH타운",
    freeParkingChargeTime: 20,
    parkingFee: "정보 없음",
    parkingFeeDiscount: "정보 없음",
    DcCount: 6,
  },
  {
    id: 57,
    locationId: "injesupercharger",
    name: "Inje, South Korea",
    status: "OPEN",
    address: {
      city: "인제군",
      state: "강원도",
      shortState: "강원",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "강원도 인제군 기린면 상하답로 130",
      locationDetail: "지상",
    },
    gps: {
      latitude: 38.0053,
      longitude: 128.289,
    },
    dateOpened: "2022-09-28",
    stallCount: 8,
    counted: true,
    elevationMeters: 293,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "인제 수퍼차저",
    commonName: "인제스피디움",
    freeParkingChargeTime: 20,
    parkingFee: "정보 없음",
    parkingFeeDiscount: null,
  },
  {
    id: 58,
    locationId: "sintanjinsupercharger",
    name: "Sintanjin, South Korea",
    status: "OPEN",
    address: {
      city: "대덕구",
      state: "대전광역시",
      shortState: "대전",
      zip: "34335",
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "대전광역시 대덕구 신탄진로 504",
      locationDetail: "지상",
    },
    gps: {
      latitude: 36.423696,
      longitude: 127.419877,
    },
    dateOpened: "2019-04-02",
    stallCount: 12,
    counted: true,
    elevationMeters: 61,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "신탄진 수퍼차저",
    commonName: "대덕 패션타운",
    freeParkingChargeTime: 10,
    parkingFee: "최초 30분 1,500원 이후 10분당 500원",
    parkingFeeDiscount: null,
  },
  {
    id: 59,
    locationId: "sejongsupercharger",
    name: "Sejong, South Korea",
    status: "OPEN",
    address: {
      city: "세종특별자치시",
      state: "세종특별자치시",
      shortState: "세종",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "세종특별자치시 다솜1로 20",
      locationDetail: "지하 2층",
    },
    gps: {
      latitude: 36.500458,
      longitude: 127.256765,
    },
    dateOpened: "2022-03-15",
    stallCount: 9,
    counted: true,
    elevationMeters: 25,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "세종 수퍼차저",
    commonName: "KT&G 세종타워C (AK PLAZA 세종 패션관)",
    freeParkingChargeTime: 30,
    parkingFee: "정보 없음",
    parkingFeeDiscount: null,
    DcCount: 3,
  },
  {
    id: 60,
    locationId: "jecheonsupercharger",
    name: "Jecheon, South Korea",
    status: "OPEN",
    address: {
      city: "제천시",
      state: "충청북도",
      shortState: "충북",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "충청북도 제천시 금성면 청풍호로 741",
      locationDetail: "지상",
    },
    gps: {
      latitude: 37.072695,
      longitude: 128.191443,
    },
    dateOpened: "2021-12-31",
    stallCount: 8,
    counted: true,
    elevationMeters: 173,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "제천 수퍼차저",
    commonName: "남제천IC 휴게소",
    freeParkingChargeTime: "free",
    parkingFee: "없음",
    parkingFeeDiscount: null,
  },
  {
    id: 61,
    locationId: "chungjusupercharger",
    name: "Chungju, South Korea",
    status: "OPEN",
    address: {
      city: "충주시",
      state: "충청북도",
      shortState: "충북",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "충청북도 충주시 대소원면 기업도시로 26",
      locationDetail: "지상",
    },
    gps: {
      latitude: 37.0016,
      longitude: 127.83573,
    },
    dateOpened: "2021-04-19",
    stallCount: 8,
    counted: true,
    elevationMeters: 0,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "충주 수퍼차저",
    commonName: "홈마트 서충주점",
    DcCount: 7,
    freeParkingChargeTime: "free",
    parkingFee: "없음",
    parkingFeeDiscount: null,
  },
  {
    id: 62,
    locationId: "jincheonsupercharger",
    name: "Jincheon, South Korea",
    status: "OPEN",
    address: {
      city: "진천군",
      state: "충청북도",
      shortState: "충북",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "충청북도 진천군 이월면 내촌리 602-1",
      locationDetail: "지상",
    },
    gps: {
      latitude: 36.9422,
      longitude: 127.43131,
    },
    dateOpened: "2021-04-30",
    stallCount: 8,
    counted: true,
    elevationMeters: 89,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "진천 수퍼차저",
    commonName: "만나 씨이에이",
    DcCount: 3,
    freeParkingChargeTime: "free",
    parkingFee: "없음",
    parkingFeeDiscount: null,
  },
  {
    id: 63,
    locationId: "cheongjusupercharger",
    name: "Cheongju, South Korea",
    status: "OPEN",
    address: {
      city: "청주시",
      state: "충청북도",
      shortState: "충북",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "충청북도 청주시 청원구 율량로 191",
      locationDetail: "2, 3층",
    },
    gps: {
      latitude: 36.66529,
      longitude: 127.4993,
    },
    dateOpened: "2022-03-30",
    stallCount: 8,
    counted: true,
    elevationMeters: 61,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "청주 수퍼차저",
    commonName: "이안스퀘어",
    freeParkingChargeTime: 0,
    parkingFee: "30분 1500원",
    parkingFeeDiscount: "주말 무료, 업장 이용 시 무료 주차",
  },
  {
    id: 64,
    locationId: "chungjumodaoutletsupercharger",
    name: "Chungju - Moda Outlet, South Korea",
    status: "OPEN",
    address: {
      city: "충주시",
      state: "충청북도",
      shortState: "충북",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "충청북도 충주시 중원대로 3683",
      locationDetail: "지상",
    },
    gps: {
      latitude: 36.961,
      longitude: 127.8998,
    },
    dateOpened: "2022-09-30",
    stallCount: 8,
    counted: true,
    elevationMeters: 74,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "모다아울렛-충주 수퍼차저",
    commonName: "모다아울렛 충주점",
    DcCount: 3,
    freeParkingChargeTime: "free",
    parkingFee: "없음",
    parkingFeeDiscount: null,
  },
  {
    id: 65,
    locationId: "dangjinsupercharger",
    name: "Dangjin, South Korea",
    status: "OPEN",
    address: {
      city: "당진시",
      state: "충청남도",
      shortState: "충남",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "충청남도 당진시 송악읍 송악로 63",
      locationDetail: "지상",
    },
    gps: {
      latitude: 36.909503,
      longitude: 126.69407466,
    },
    dateOpened: "2019-12-20",
    stallCount: 6,
    counted: true,
    elevationMeters: 42,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "당진 수퍼차저",
    commonName: "커피란 워시데이 당진점",
    freeParkingChargeTime: "free",
    parkingFee: null,
    parkingFeeDiscount: null,
  },
  {
    id: 66,
    locationId: "nonsansupercharger",
    name: "Nonsan, South Korea",
    status: "OPEN",
    address: {
      city: "논산시",
      state: "충청남도",
      shortState: "충남",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "충청남도 논산시 연무읍 동안로1113번길 36",
      locationDetail: "지상",
    },
    gps: {
      latitude: 36.12882428,
      longitude: 127.12962019,
    },
    dateOpened: "2019-06-20",
    stallCount: 6,
    counted: true,
    elevationMeters: 29,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "논산 수퍼차저",
    commonName: "논산 라온빌리지",
    freeParkingChargeTime: "free",
    parkingFee: null,
    parkingFeeDiscount: null,
  },
  {
    id: 67,
    locationId: "buyeosupercharger",
    name: "Buyeo, South Korea",
    status: "OPEN",
    address: {
      city: "부여군",
      state: "충청남도",
      shortState: "충남",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "충청남도 부여군 규암면 백제문로 387",
      locationDetail: "1층",
    },
    gps: {
      latitude: 36.306315,
      longitude: 126.900404,
    },
    dateOpened: "2022-03-15",
    stallCount: 8,
    counted: true,
    elevationMeters: 21,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "부여 수퍼차저",
    commonName: "롯데아울렛 부여점",
    freeParkingChargeTime: "free",
    parkingFee: "없음",
    parkingFeeDiscount: null,
    DcCount: 5,
  },
  {
    id: 68,
    locationId: "busangangseosupercharger",
    name: "Busan - Gangseo, South Korea",
    status: "OPEN",
    address: {
      city: "강서구",
      state: "부산광역시",
      shortState: "부산",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "부산광역시 강서구 대저1동 361-68번지",
      locationDetail: "지상",
    },
    gps: {
      latitude: 35.219392,
      longitude: 128.988331,
    },
    dateOpened: "2019-12-24",
    stallCount: 6,
    counted: true,
    elevationMeters: 6,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "부산 - 강서 수퍼차저",
    commonName: "부산 응카",
    freeParkingChargeTime: "free",
    parkingFee: "없음",
    parkingFeeDiscount: null,
  },
  {
    id: 69,
    locationId: "busannamgusupercharger",
    name: "Busan - Nam-gu, South Korea",
    status: "OPEN",
    address: {
      city: "남구",
      state: "부산광역시",
      shortState: "부산",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "부산광역시 남구 전포대로 133",
      locationDetail: "지하 3층",
    },
    gps: {
      latitude: 35.148279,
      longitude: 129.065439,
    },
    dateOpened: "2019-12-24",
    stallCount: 4,
    counted: true,
    elevationMeters: 16,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "부산 - 남구 수퍼차저",
    commonName: "IFC 부산몰",
    freeParkingChargeTime: 20,
    parkingFee: "10분당 500원",
    parkingFeeDiscount: null,
  },
  {
    id: 70,
    locationId: "busansupercharger",
    name: "Busan, South Korea",
    status: "OPEN",
    address: {
      city: "해운대구",
      state: "부산광역시",
      shortState: "부산",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "부산광역시 해운대구 해운대해변로 296",
      locationDetail: "지상",
    },
    gps: {
      latitude: 35.160676,
      longitude: 129.163823,
    },
    dateOpened: "2017-07-17",
    stallCount: 6,
    counted: true,
    elevationMeters: 17,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "부산 수퍼차저",
    commonName: "파라다이스호텔 부산",
    freeParkingChargeTime: 120,
    parkingFee: "2시간 무료, 10분당 1,000원",
    parkingFeeDiscount: null,
  },
  {
    id: 71,
    locationId: "busanyeonjesupercharger",
    name: "Busan - Yeonje, South Korea",
    status: "OPEN",
    address: {
      city: "연제구",
      state: "부산광역시",
      shortState: "부산",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "부산광역시 연제구 좌수영로 290",
      locationDetail: "지상",
    },
    gps: {
      latitude: 35.188372,
      longitude: 129.110238,
    },
    dateOpened: "2020-09-19",
    stallCount: 4,
    counted: true,
    elevationMeters: 12,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "부산–연제 수퍼차저",
    commonName: "부산서비스센터",
    freeParkingChargeTime: "free",
    parkingFee: "무료",
    parkingFeeDiscount: null,
  },
  {
    id: 72,
    locationId: "busangijangsupercharger",
    name: "Busan - Gijang, South Korea",
    status: "OPEN",
    address: {
      city: "기장군",
      state: "부산광역시",
      shortState: "부산",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "부산광역시 기장군 기장읍 기장해안로 147",
      locationDetail: "지상",
    },
    gps: {
      latitude: 35.19304,
      longitude: 129.21153,
    },
    dateOpened: "2022-09-02",
    stallCount: 12,
    counted: true,
    elevationMeters: 31,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "부산–기장 수퍼차저",
    commonName: "롯데프리미엄아울렛 동부산점",
    freeParkingChargeTime: "free",
    parkingFee: "무료",
    parkingFeeDiscount: null,
    DcCount: 6,
  },
  {
    id: 73,
    locationId: "teslabusanhotelnongshimsupercharger",
    name: "Busan - Dongnae, South Korea",
    status: "OPEN",
    address: {
      city: "동래구",
      state: "부산광역시",
      shortState: "부산",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "부산 동래구 금강공원로20번길 23",
      locationDetail: "지상 4층",
    },
    gps: {
      latitude: 35.2197,
      longitude: 129.083,
    },
    dateOpened: "2022-09-30",
    stallCount: 6,
    counted: true,
    elevationMeters: 14,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "동래 호텔농심 수퍼차저",
    commonName: "농심 호텔",
    freeParkingChargeTime: 0,
    parkingFee: "10분당 1,000원",
    parkingFeeDiscount: "1만원 이용 시 1시간 무료",
  },
  {
    id: 74,
    locationId: "teslabusanhaewondaelottedepartmentcentumcitysupercharger",
    name: "Busan - Centum City, South Korea",
    status: "OPEN",
    address: {
      city: "해운대구",
      state: "부산광역시",
      shortState: "부산",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "부산 해운대구 센텀남대로 59",
      locationDetail: "지하 5층",
    },
    gps: {
      latitude: 35.1696,
      longitude: 129.1312,
    },
    dateOpened: "2022-09-30",
    stallCount: 6,
    counted: true,
    elevationMeters: 1,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "센텀시티 수퍼차저",
    commonName: "롯데백화점 센텀시티점",
    freeParkingChargeTime: 0,
    parkingFee: "정보 없음",
    parkingFeeDiscount: null,
  },
  {
    id: 75,
    locationId: "daegusupercharger",
    name: "Daegu, South Korea",
    status: "OPEN",
    address: {
      city: "북구",
      state: "대구광역시",
      shortState: "대구",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "대구광역시 북구 엑스코로 10",
      locationDetail: "신관 지하 1층",
    },
    gps: {
      latitude: 35.907022,
      longitude: 128.613115,
    },
    dateOpened: "2017-06-19",
    stallCount: 6,
    counted: true,
    elevationMeters: 35,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "대구 수퍼차저",
    commonName: "대구 엑스코",
    freeParkingChargeTime: 60,
    parkingFee: "10분당 300원",
    parkingFeeDiscount: null,
  },
  {
    id: 76,
    locationId: "daegueastsupercharger",
    name: "Daegu - Isiapolis, South Korea",
    status: "OPEN",
    address: {
      city: "동구",
      state: "대구광역시",
      shortState: "대구",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "대구광역시 동구 팔공로49길 16",
      locationDetail: "지상 1층",
    },
    gps: {
      latitude: 35.9229,
      longitude: 128.63571,
    },
    dateOpened: "2021-06-25",
    stallCount: 6,
    counted: true,
    elevationMeters: 27,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "대구 - 이시아폴리스 수퍼차저",
    commonName: "롯데아울렛 이시아폴리스점",
    freeParkingChargeTime: 30,
    parkingFeeDiscount:
      "1만원 구매 시 1시간 무료, 식당가/F&B 사용 시 2시간 무료",
    parkingFee: "10분당 500원",
  },
  {
    id: 77,
    locationId: "daegususeongsupercharger",
    name: "Daegu-Suseong, South Korea",
    status: "OPEN",
    address: {
      city: "수성구",
      state: "대구광역시",
      shortState: "대구",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "대구광역시 수성구 수성못6길 18",
      locationDetail: "지상",
    },
    gps: {
      latitude: 35.82726,
      longitude: 128.62369,
    },
    dateOpened: "2022-04-01",
    stallCount: 8,
    counted: true,
    elevationMeters: 76,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "대구-수성 수퍼차저",
    commonName: "대구수성 아트스퀘어",
    freeParkingChargeTime: 10,
    parkingFee: "최초 30분 1,000원, 이후 10분당 500원",
    parkingFeeDiscount: null,
    DcCount: 3,
  },
  {
    id: 78,
    locationId: "sangjusupercharger",
    name: "Sangju, South Korea",
    status: "OPEN",
    address: {
      city: "상주시",
      state: "경상북도",
      shortState: "경북",
      zip: "37132",
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경상북도 상주시 공검면 경상대로 4207",
      locationDetail: "지상",
    },
    gps: {
      latitude: 36.515931,
      longitude: 128.1634319,
    },
    dateOpened: "2018-12-13",
    stallCount: 12,
    counted: true,
    elevationMeters: 70,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "상주 수퍼차저",
    commonName: "연꽃 참한우",
    parkingFee: "없음",
    freeParkingChargeTime: "free",
    parkingFeeDiscount: null,
  },
  {
    id: 79,
    locationId: "cheongdosupercharger",
    name: "Cheongdo, South Korea",
    status: "OPEN",
    address: {
      city: "청도군",
      state: "경상북도",
      shortState: "경북",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경상북도 청도군 청도읍 새마을로 1376",
      locationDetail: "지상",
    },
    gps: {
      latitude: 35.654083,
      longitude: 128.749412,
    },
    dateOpened: "2021-12-31",
    stallCount: 6,
    counted: true,
    elevationMeters: 72,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "청도 수퍼차저",
    commonName: "청도 이마트24 청도IC점",
    freeParkingChargeTime: "free",
    parkingFee: "없음",
    parkingFeeDiscount: null,
  },
  {
    id: 80,
    locationId: "gyeongjusupercharger",
    name: "Gyeongju, South Korea",
    status: "OPEN",
    address: {
      city: "경주시",
      state: "경상북도",
      shortState: "경북",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경상북도 경주시 보문로 338",
      locationDetail: "외부 지상 주차장",
    },
    gps: {
      latitude: 35.850434,
      longitude: 129.277112,
    },
    dateOpened: "2022-07-02",
    stallCount: 6,
    counted: true,
    elevationMeters: 107,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "경주 수퍼차저",
    commonName: "라한셀렉트 경주",
    freeParkingChargeTime: 0,
    parkingFee: "정보 없음",
    parkingFeeDiscount: null,
  },
  {
    id: 81,
    locationId: "pohangsupercharger",
    name: "Pohang, South Korea",
    status: "OPEN",
    address: {
      city: "포항시",
      state: "경상북도",
      shortState: "경북",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경상북도 포항시 북구 흥해읍 동해대로 878",
      locationDetail: "지상",
    },
    gps: {
      latitude: 36.059093,
      longitude: 129.320555,
    },
    dateOpened: "2022-04-01",
    stallCount: 12,
    counted: true,
    elevationMeters: 104,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "포항 수퍼차저",
    commonName: "포항 오토파크",
    DcCount: 18,
    freeParkingChargeTime: "free",
    parkingFee: "없음",
    parkingFeeDiscount: null,
  },
  {
    id: 82,
    locationId: "hamyangsupercharger",
    name: "Hamyang, South Korea",
    status: "OPEN",
    address: {
      city: "함양군",
      state: "경상남도",
      shortState: "경남",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경상남도 함양군 함양읍 고운로 298",
      locationDetail: "지상",
    },
    gps: {
      latitude: 35.524635,
      longitude: 127.75314,
    },
    dateOpened: "2019-06-28",
    stallCount: 6,
    counted: true,
    elevationMeters: 180,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "함양 수퍼차저",
    commonName: "타이어프로 함양IC점",
    freeParkingChargeTime: "free",
    parkingFee: "없음",
    parkingFeeDiscount: null,
  },
  {
    id: 83,
    locationId: "jinjusksupercharger",
    name: "Jinju, South Korea",
    status: "OPEN",
    address: {
      city: "진주시",
      state: "경상남도",
      shortState: "경남",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경상남도 진주시 동진로 440",
      locationDetail: "지하 2층",
    },
    gps: {
      latitude: 35.1802627,
      longitude: 128.1397201,
    },
    dateOpened: "2018-12-12",
    stallCount: 6,
    counted: true,
    elevationMeters: 21,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "진주 수퍼차저",
    commonName: "롯데몰 진주점",
    freeParkingChargeTime: "free",
    parkingFee: "없음",
    parkingFeeDiscount: null,
  },
  {
    id: 84,
    locationId: "changwonsupercharger",
    name: "Changwon, South Korea",
    status: "OPEN",
    address: {
      city: "창원시",
      state: "경상남도",
      shortState: "경남",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경상남도 창원시 성산구 중앙대로 100번길 13",
      locationDetail: "지하 2층",
    },
    gps: {
      latitude: 35.22269,
      longitude: 128.68126,
    },
    dateOpened: "2022-04-01",
    stallCount: 6,
    counted: true,
    elevationMeters: 19,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "창원 수퍼차저",
    commonName: "상남 CGV",
    freeParkingChargeTime: 10,
    parkingFee: "최초 30분 1,500원 이후 10분당 500원",
    parkingFeeDiscount: "CGV 이용 시 3시간 무료",
  },
  {
    id: 85,
    locationId: "hadongupercharger",
    name: "Hadong, South Korea",
    status: "OPEN",
    address: {
      city: "하동군",
      state: "경상남도",
      shortState: "경남",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경상남도 하동군 진교면 강변길 13-1",
      locationDetail: "지상",
    },
    gps: {
      latitude: 35.03,
      longitude: 127.9039,
    },
    dateOpened: "2022-09-30",
    stallCount: 8,
    counted: true,
    elevationMeters: 1,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "하동 수퍼차저",
    commonName: "진교면 공영 주차장",
    freeParkingChargeTime: "free",
    parkingFee: "없음",
    parkingFeeDiscount: null,
  },
  {
    id: 86,
    locationId: "gwangjusupercharger",
    name: "Gwangju, South Korea",
    status: "OPEN",
    address: {
      city: "서구",
      state: "광주광역시",
      shortState: "광주",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "광주광역시 서구 상무누리로 55",
      locationDetail: "지하 1층",
    },
    gps: {
      latitude: 35.147921,
      longitude: 126.837407,
    },
    dateOpened: "2017-10-09",
    stallCount: 6,
    counted: true,
    elevationMeters: 15,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "광주 수퍼차저",
    commonName: "홀리데이인 광주",
    freeParkingChargeTime: 20,
    parkingFee: "30분당 1,000원",
    parkingFeeDiscount: null,
  },
  {
    id: 87,
    locationId: "gwangjucheomdansupercharger",
    name: "Gwangju - Cheomdan, South Korea",
    status: "OPEN",
    address: {
      city: "광산구",
      state: "광주광역시",
      shortState: "광주",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "광주광역시 광산구 임방울대로826번길 19-20",
      locationDetail: "지상 6층",
    },
    gps: {
      latitude: 35.216576,
      longitude: 126.849241,
    },
    dateOpened: "2022-06-13",
    stallCount: 6,
    counted: true,
    elevationMeters: 24,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "광주 - 첨단 수퍼차저",
    commonName: "보이저 첨단",
    freeParkingChargeTime: 0,
    parkingFee: "30분당 2,000원",
    parkingFeeDiscount: null,
  },
  {
    id: 88,
    locationId: "gunsansupercharger",
    name: "Gunsan, South Korea",
    status: "OPEN",
    address: {
      city: "군산시",
      state: "전라북도",
      shortState: "전북",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "전라북도 군산시 대야면 번영로 1321",
      locationDetail: "지상",
    },
    gps: {
      latitude: 35.937086,
      longitude: 126.855631,
    },
    dateOpened: "2019-07-02",
    stallCount: 8,
    counted: true,
    elevationMeters: 8,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "군산 수퍼차저",
    commonName: "군산 GS 칼텍스 샛별 주유소",
    freeParkingChargeTime: "free",
    parkingFee: "없음",
    parkingFeeDiscount: null,
  },
  {
    id: 89,
    locationId: "jeonjusupercharger",
    name: "Jeonju, South Korea",
    status: "OPEN",
    address: {
      city: "전주시",
      state: "전라북도",
      shortState: "전북",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "전라북도 전주시 완산구 용머리로 45",
      locationDetail: "지하 2층",
    },
    gps: {
      latitude: 35.80698,
      longitude: 127.11566,
    },
    dateOpened: "2022-04-01",
    stallCount: 8,
    counted: true,
    elevationMeters: 30,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "전주 수퍼차저",
    commonName: "전주 효자몰",
    freeParkingChargeTime: 0,
    parkingFee: "정보 없음",
    parkingFeeDiscount: null,
    DcCount: 3,
  },
  {
    id: 90,
    locationId: "imsilcheesepark",
    name: "Imsil Cheese Theme Park, South Korea",
    status: "OPEN",
    address: {
      streetName:
        "Imsil Cheese Theme Park 50, Doin 2-gil, Seongsu-myeon, Imsil-gun",
      city: "임실군",
      state: "전라북도",
      shortState: "전북",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      locationDetail: "지상",
    },
    gps: {
      latitude: 35.63068,
      longitude: 127.303884,
    },
    dateOpened: "2022-11-29",
    stallCount: 6,
    counted: true,
    elevationMeters: 230,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    commonName: "임실 슈퍼차저",
    KRName: "임실 치즈 테마파크",
    freeParkingChargeTime: "free",
    parkingFee: "무료",
    parkingFeeDiscount: null,
  },
  {
    id: 91,
    locationId: "yeosusupercharger",
    name: "Yeosu, South Korea",
    status: "OPEN",
    address: {
      city: "여수시",
      state: "전라남도",
      shortState: "전남",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "전라남도 여수시 신월로 496-25",
      locationDetail: "지상",
    },
    gps: {
      latitude: 34.72179,
      longitude: 127.69945,
    },
    dateOpened: "2021-12-10",
    stallCount: 8,
    counted: true,
    elevationMeters: 10,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "여수 수퍼차저",
    commonName: "여수 히든베이호텔",
    freeParkingChargeTime: "free",
    parkingFee: "없음",
    parkingFeeDiscount: null,
  },
  {
    id: 92,
    locationId: "suncheonsupercharger",
    name: "Suncheon, South Korea",
    status: "OPEN",
    address: {
      city: "순천시",
      state: "전라남도",
      shortState: "전남",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "전라남도 순천시 조례동 640",
      locationDetail: "지상",
    },
    gps: {
      latitude: 34.96316,
      longitude: 127.52554,
    },
    dateOpened: "2021-07-01",
    stallCount: 8,
    counted: true,
    elevationMeters: 25,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "순천 수퍼차저",
    commonName: "호수공원 외식타운",
    freeParkingChargeTime: 0,
    parkingFee: "10분당 1,000원",
    parkingFeeDiscount: "업장 이용 시 2시간 무료 \n 23시~9시 무료",
    DcCount: 4,
  },
  {
    id: 93,
    locationId: "gwangyangsupercharger",
    name: "Gwangyang, South Korea",
    status: "OPEN",
    address: {
      city: "광양시",
      state: "전라남도",
      shortState: "전남",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "전라남도 광양시 순광로 466",
      locationDetail: "지상 2층",
    },
    gps: {
      latitude: 34.961728,
      longitude: 127.565446,
    },
    dateOpened: "2022-05-27",
    stallCount: 6,
    counted: true,
    elevationMeters: 13,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "광양 수퍼차저",
    commonName: "LF스퀘어 광양테라스몰",
    freeParkingChargeTime: "free",
    parkingFee: "없음",
    parkingFeeDiscount: null,
  },
  {
    id: 94,
    locationId: "seogwiposupercharger",
    name: "Seogwipo, South Korea",
    status: "OPEN",
    address: {
      city: "서귀포시",
      state: "제주특별자치도",
      shortState: "제주",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "제주특별자치도 서귀포시 중문관광로72번길 35",
      locationDetail: "지상",
    },
    gps: {
      latitude: 33.249844,
      longitude: 126.409044,
    },
    dateOpened: "2017-12-12",
    stallCount: 12,
    counted: true,
    elevationMeters: 67,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    KRName: "서귀포 수퍼차저",
    commonName: "롯데호텔 제주 호텔주차장",
    freeParkingChargeTime: 30,
    parkingFee: "30분당 1,000원",
    parkingFeeDiscount: null,
  },
  {
    id: 95,
    locationId: "jejusupercharger",
    name: "Jeju, South Korea",
    status: "OPEN",
    address: {
      city: "제주시",
      state: "제주특별자치도",
      shortState: "제주",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "제주특별자치도 제주시 연삼로 136",
      locationDetail: "지상",
    },
    gps: {
      latitude: 33.492611,
      longitude: 126.5071,
    },
    dateOpened: "2021-10-06",
    stallCount: 8,
    counted: true,
    elevationMeters: 76,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "제주 수퍼차저",
    commonName: "제주 테슬라 스토어",
    freeParkingChargeTime: "free",
    parkingFee: "없음",
    parkingFeeDiscount: null,
  },
  {
    id: 5574,
    locationId: "31209",
    name: "Sejong - Daepyeong, South Korea",
    status: "OPEN",
    address: {
      streetName: "Sejong Jonghabundongjang-ro 29",
      city: "세종특별자치시",
      state: "세종특별자치시",
      shortState: "세종",
      zip: "30154",
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      locationDetail: "지상",
    },
    gps: { latitude: 36.468663, longitude: 127.267814 },
    dateOpened: "2022-11-29",
    stallCount: 6,
    counted: true,
    elevationMeters: 20,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    freeParkingChargeTime: "free",
    parkingFee: "없음",
    parkingFeeDiscount: null,
    KRName: "세종 SK 대평 주유소",
    commonName: "세종 슈퍼차저",
  },
  {
    id: 5053,
    locationId: "35418",
    name: "Sokcho - Hanwha Resort, South Korea",
    status: "OPEN",
    address: {
      streetName: "88, Misiryeong-ro 2983beon-gil, Sokcho-si, Gangwon-do",
      city: "속초",
      state: "강원도",
      shortState: "강원",
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      locationDetail: "지상",
    },
    gps: { latitude: 38.2081, longitude: 128.5265 },
    dateOpened: "2022-09-30",
    stallCount: 6,
    counted: true,
    elevationMeters: 77,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    freeParkingChargeTime: "free",
    parkingFee: "무료",
    parkingFeeDiscount: null,
    DcCount: 3,
    KRName: "속초 설악 슈퍼차저",
    commonName: "설악 워터피아",
  },
  {
    id: 5508,
    locationId: "Yangsansupercharger",
    name: "Yangsan, South Korea",
    status: "OPEN",
    address: {
      city: "양산시",
      state: "경상남도",
      shortState: "경남",
      zip: "50611",
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      streetName: "경상남도 양산시 물금읍 범어리 2710-1",
      locationDetail: "지하 2층",
    },
    gps: { latitude: 35.32755, longitude: 129.011743 },
    dateOpened: "2022-11-10",
    stallCount: 6,
    counted: true,
    elevationMeters: 3,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    KRName: "양산 슈퍼차저",
    commonName: "양산 오슬로 파크",
    freeParkingChargeTime: 20,
    parkingFee:
      "최초 30분 1,000원 이후 10분당 500원 \n 21시~09시 시간당 1,000원",
    parkingFeeDiscount: "매장 이용 시, 2시간 무료",
  },
  {
    id: 1116,
    locationId: "cheonansupercharger",
    name: "Cheonan, South Korea",
    status: "OPEN",
    address: {
      streetName: "충청남도 천안시 동남구 성남면 종합휴양지로 200",
      city: "천안시",
      state: "충청남도",
      shortState: "충남",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
    },
    gps: { latitude: 36.757187, longitude: 127.224121 },
    dateOpened: "2017-05-31",
    stallCount: 6,
    counted: true,
    elevationMeters: 105,
    powerKilowatt: 120,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    commonName: "천안 대명 리조트",
    KRName: "천안 슈퍼차저",
    freeParkingChargeTime: "free",
    parkingFee: "없음",
    parkingFeeDiscount: null,
  },
  {
    id: 3886,
    locationId: "daejeonsupercharger",
    name: "Daejeon, South Korea",
    status: "OPEN",
    address: {
      streetName: "대전광역시 서구 계룡로 598",
      city: "서구",
      state: "대전광역시",
      shortState: "대전",
      zip: null,
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      locationDetail: "B4F 401",
    },
    gps: {
      latitude: 36.34078,
      longitude: 127.389478,
    },
    dateOpened: "2021-12-10",
    stallCount: 6,
    counted: true,
    elevationMeters: 63,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: true,
    freeParkingChargeTime: 30,
    DcCount: 4,
    KRName: "대전 슈퍼차저",
    commonName: "서구 롯데백화점",
    parkingFee: "정보 없음",
    parkingFeeDiscount: null,
  },
  {
    id: 5507,
    locationId: "34277",
    name: "Gimcheon - Moda Outlet, South Korea",
    status: "OPEN",
    address: {
      streetName: "경상북도 김천시 아포읍 아포대로 1417",
      city: "김천시",
      state: "경상남도",
      shortState: "경남",
      zip: "39676",
      countryId: 137,
      country: "South Korea",
      regionId: 102,
      region: "Asia Pacific",
      locationDetail: "지상",
    },
    gps: { latitude: 36.15525, longitude: 128.280846 },
    dateOpened: "2022-11-10",
    stallCount: 8,
    counted: true,
    elevationMeters: 74,
    powerKilowatt: 250,
    solarCanopy: false,
    battery: false,
    statusDays: 0,
    urlDiscuss: false,
    freeParkingChargeTime: "free",
    parkingFee: "없음",
    parkingFeeDiscount: null,
    KRName: "김천 슈퍼차저",
    commonName: "모다아울렛 김천구미점",
  },
]

export default superChargerInfo
